...
var sifra = "0";
var token = "0";
var imeP = "0",nadimakP = "0",prezimeP = "0",emailP = "0",sifraP = "0";
var projekatBR = "0";
...
var funkicjeMicko = {
	...
	PrijavaNaServer : function() {
		...
		sifraP = $("#sifraP").val();
				...
				$.ajax({
					type: 'POST',
					url: 'http://localhost:3090/PrijavaKorisnika',
					data: { 
							'ime': imeP, 
							'nadimak': nadimakP,
							'prezime': prezimeP,
	...
	ProveraToken : function(){
		...
		$.ajax({
				...
				url: "http://localhost:3090/Sifra-Micko",
				type: "GET",
				headers:{
	...
	InsertValues :  function(){
		...
		$.ajax({
			url: "http://localhost:3090/ListaProjekata",
            type: "GET",
			headers: {
				"Authorization": "Bearer " + token
			},
					...
					options = '';
					    var optionFound = false,
							...
							if (optionFound) {
									...
									//alert(mesecInf);
									/*if(nedeljaMicko == "" && flgTrenutneNedelje1 == 0)
									{
										nedeljaMicko = funkicjeMicko.TrenutnaNedelja_nn();
										flgTrenutneNedelje1 = 1;
									...
									funkicjeMicko.Trenutna_nedelja();
                                    if(flgMeseci1 == 1 && flgNed1 == 1 && flgProjekat1 == 1){
									...
									//$("#Mesec").val("Nepostojeca vrednost");
							}
	...
	Upis_Stanja :  function(){
		...
		//alert(nadimakBaza2);
		//var projektiBaza = $("#projekti1").val();
	    //alert(projektiBaza);
		//var mesecBaza = $("#Mesec").val();
		//alert(mesecBaza);
		//var nedeljaBaza = $("#odabrananedelja").val();
		//alert(nedeljaBaza);
		//var brojsatiBaza = $("#brojsati").val();
		//alert(brojsatiBaza);
		//alert(godinaInf);
        ...
        else if(flgZ3 == 1)
        {
			...
			//alert("Micko");
			$.ajax({
				url: "http://localhost:3090/micko/Stanje-Procedure",
				type: "Post",
				headers: {
					"Authorization": "Bearer " + token
				},
	...
	Promena_sifre: function(){
		...
		if (sifra_nova == poredi)
		{
			...
			$.ajax({
				...
				type: 'POST',
				url: 'http://localhost:3090/Dva-upita-promena-sifre?imeSifra='+ime_lozinka+'&sifra='+sifra_promeni+'',
				headers:{
	...
	Trenutna_nedelja: function(){
			...
			$.ajax({
				...
				url: 'http://localhost:3090/Djuture-upis?nadimak='+ime+'',
				type: "get",
				headers:{
						...
						var presidentTabela = respObjTebela[i];
							...
							{   // create new data cell for names
								...
								if(brojacMicko==2)
								{
									...
									/*for(var p = 0; p < 1 ; p++)
									{
										  alert("brojanje"+brojanje);
									      NizProjekti[brojanje] = 0;
										  alert(NizProjekti[brojanje]);
										  //$("#inputTrenutnaNedelja"+idUvecajInput1+"2").val(0);
										  //cell.value = 0;
	...
	KlikNedeljeTrenutne: function(){
		...
		//alert(o);
		//alert(o1);
		alert("BrojacProjekata"+BrojacProjekata);
		alert("brojanje"+brojanje);
		...
		{			
			...
			//alert(nadimakBaza2);
			//alert(PakovanjeProjekti[i]);
			//alert(trenutnaNedelja_Upit);
			//alert(trenutnaMesec_Upit);
			//alert(Pakovanje[i]);
			...
			var optionFound = false,
				...
				for (var j = 0; j < datalist.options.length; j++) {
					...
					if (optionFound) {
						...
						funkicjeMicko.Trenutna_nedelja();
						funkicjeCile.Izvestaj1();
						if(flgNed1 == 1 && flgMeseci1 == 1 &&  flgProjekat1 == 1 && flgneki == 1){
							
							document.getElementById("UpisUBazu").disabled = false;
						}
						else if(flgNed1 == 1 && flgMeseci1 == 1 &&  flgProjekat1 == 1 ){
